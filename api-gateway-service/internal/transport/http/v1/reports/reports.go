...
type reportsHandler struct {
	natsHandlers *reports.ReportsHandler
	jwtKey       string
}
...
type (
	...
	getAllByDeviceIdResp struct {
		Data pbmessages.ReportGetAllByDeviceIdResp `json:"data"`
	}
)
...
func (h *reportsHandler) getAllByDeviceId(ctx fiber.Ctx) error {
	...
	res, err := h.natsHandlers.PublishGetAllByDeviceId(
		pbmessages.ReportGetAllByDeviceIdReq{
			DeviceId: int32(body.DeviceID),
		},
	)
	if err != nil {
		return fiber.NewError(
...
type (
	...
	getAllByPeriodResp struct {
		Data pbmessages.ReportGetAllByPeriodResp `json:"data"`
	}
)
...
func (h *reportsHandler) getAllByPeriod(ctx fiber.Ctx) error {
	...
	res, err := h.natsHandlers.PublishGetAllByPeriod(
		pbmessages.ReportGetAllByPeriodReq{
			StartTime: timestamppb.New(body.StartTime),
...
type (
	...
	getCountByMessageTypeResp struct {
		Data pbmessages.ReportGetCountByMessageTypeResp `json:"data"`
	}
)
...
func (h *reportsHandler) getCountByMessageType(ctx fiber.Ctx) error {
	...
	res, err := h.natsHandlers.PublishGetCountByMessageType(
		pbmessages.ReportGetCountByMessageTypeReq{
...
type (
	getMonthReportResp struct {
		Data pbmessages.MonthReport `json:"data"`
	}
)

func (h *reportsHandler) getMonthReport(ctx fiber.Ctx) error {
	res, err := h.natsHandlers.PublishGetMonthReport()
	if err != nil {
		return fiber.NewError(
			fiber.StatusInternalServerError,
